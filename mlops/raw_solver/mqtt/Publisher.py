...
DEFAULT_CSV = ROOT / "data" / "input_gradual.csv"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=env_int(
            "PRESS_LINE_NUMBER",
            int(settings.get("line_number", 7)),
        ),
        help="데이터를 보낼 공장 라인 번호(1~16, 기본값: 7/C3)",
    )
    ...
    if args.line_number < 1 or args.line_number > 16:
        parser.error("--line-number 값은 1~16이어야 합니다")
    return args
...
def read_csv(path: Path) -> tuple[list[str], list[dict[str, str]]]:
    ...
    # --csv에 폴더가 전달되면 기본 파일을 먼저 찾고, 기본 파일이 없을 때는
    # 폴더 안의 유일한 CSV 파일을 자동으로 선택합니다.
    if path.is_dir():
        default_candidate = path / DEFAULT_CSV.name
        if default_candidate.is_file():
            path = default_candidate
        else:
            csv_candidates = sorted(path.glob("*.csv"))
            if not csv_candidates:
                raise FileNotFoundError(f"폴더에 CSV 파일이 없습니다: {path}")
            if len(csv_candidates) > 1:
                candidate_names = ", ".join(candidate.name for candidate in csv_candidates)
                raise ValueError(
                    f"폴더에 CSV 파일이 여러 개 있습니다. --csv에 파일을 지정하세요: "
                    f"{candidate_names}"
                )
...
def main() -> int:
    ...
    publish_columns = list(columns)
    if "line_number" not in publish_columns:
        publish_columns.append("line_number")
    ...
    try:
        ...
        for _, row in iter_replayed_rows(rows, args.repeat):
            ...
            sequence = sent + 1
            row["line_number"] = str(args.line_number)
