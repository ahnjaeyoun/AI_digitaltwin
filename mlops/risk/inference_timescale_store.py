...
class InferenceTimescaleStore:
        ...
        for table_name in ("press_anomaly_results", "press_risk_results"):
            self.connection.execute(
                sql.SQL(
                    ...
                    "if_not_exists => TRUE)"
                ).format(sql.Literal(f"{self.schema}.{table_name}"))
            )
    ...
    def _insert_anomaly(self, values: dict[str, Any]) -> None:
        ...
        self.connection.execute(
            ...
            (
                values["event_time"], values["run_id"], values["message_id"],
                values["source_message_id"], values["input_row_index"],
                values["line_number"], values["cycle_id"], values["cycle_phase"],
                values["status"], _number(inference.get("recon_error")),
                _number(inference.get("score")), inference.get("is_anomaly"),
    ...
    def _insert_risk(self, values: dict[str, Any]) -> None:
        ...
        self.connection.execute(
            ...
            (
                ...
                values["source_message_id"], values["input_row_index"],
                values["line_number"], values["cycle_id"], values["cycle_phase"],
                values["status"], _number(risk.get("score_percent")),
                _number(risk.get("linear_score_percent")),
                _number(risk.get("reconstruction_error")), risk.get("alert"),
                _integer(risk.get("window_rows")),
                _number(risk.get("history_coverage_percent")),
                _number(risk.get("p95")), _number(risk.get("p99")),
    ...
    def flush_pending(self, *, force: bool = False) -> int:
        ...
        try:
            ...
            if message != self.last_error:
                print(
                    "[TIMESCALEDB_저장대기] "
                    f"종류={self.result_kind} 대기건수={len(self.pending)} 오류={message}",
                    file=sys.stderr,
                    flush=True,
                )
